"""Build the deterministic AWS Lambda deployment package for GHSA Silver."""
...
BUILD_DIR = PROJECT_ROOT / "build" / "lambda" / "ghsa-silver"
PACKAGE_DIR = BUILD_DIR / "package"
REQUIREMENTS_FILE = BUILD_DIR / "requirements.txt"

DIST_DIR = PROJECT_ROOT / "dist"
ARTIFACT_PATH = DIST_DIR / "opslens-ghsa-silver.zip"

PYTHON_VERSION = "3.13"
PYTHON_PLATFORM = "x86_64-manylinux_2_28"
RUNTIME_DEPENDENCY_GROUP = "ghsa-silver-runtime"
...
SOURCE_MANIFEST = (
    ...
    (
        PROJECT_ROOT / "src" / "opslens" / "ingestion" / "ghsa" / "__init__.py",
        Path("opslens/ingestion/ghsa/__init__.py"),
    ),
    (
        PROJECT_ROOT / "src" / "opslens" / "ingestion" / "ghsa" / "application",
        Path("opslens/ingestion/ghsa/application"),
    ),
    (
        PROJECT_ROOT / "src" / "opslens" / "ingestion" / "ghsa" / "domain",
        Path("opslens/ingestion/ghsa/domain"),
    ),
    (
    ...
    (
        PROJECT_ROOT / "src" / "opslens" / "transformation" / "ghsa",
        Path("opslens/transformation/ghsa"),
    ),
    (
...
REQUIRED_PACKAGE_FILES = (
    Path("opslens/ingestion/ghsa/application/attempt.py"),
    Path("opslens/ingestion/ghsa/application/manifest.py"),
    Path("opslens/ingestion/ghsa/domain/api_page.py"),
    Path("opslens/transformation/ghsa/lambda_handler.py"),
    Path("opslens/transformation/ghsa/composition.py"),
    Path("opslens/transformation/ghsa/config.py"),
    Path("opslens/transformation/ghsa/adapters/inbound/invocation.py"),
    Path("opslens/transformation/ghsa/adapters/outbound/s3_exact_object.py"),
    Path("opslens/transformation/ghsa/adapters/outbound/s3_silver_content.py"),
    Path("opslens/transformation/ghsa/adapters/outbound/s3_silver_completion.py"),
    Path("opslens/transformation/ghsa/runtime/service.py"),
    Path("opslens/transformation/ghsa/runtime/processor.py"),
    Path("opslens/transformation/ghsa/completion/service.py"),
    Path("opslens/transformation/ghsa/serialization/parquet.py"),
    Path("opslens/shared/observability/powertools.py"),
    Path("pyarrow/__init__.py"),
)

FORBIDDEN_PACKAGE_FILES = (
    Path("opslens/ingestion/ghsa/lambda_handler.py"),
    Path("opslens/ingestion/ghsa/composition.py"),
)
...
def export_runtime_dependencies() -> None:
    """Export locked GHSA Silver runtime dependencies."""
    run_command(
        [
            "uv",
            "export",
...
def copy_application_source() -> None:
    """Copy only application source required by GHSA Silver."""
    for source, relative_destination in SOURCE_MANIFEST:
        if not source.exists():
            raise FileNotFoundError(
                "GHSA Silver Lambda source manifest entry does not exist: "
                f"{source}"
            )
...
def validate_package_contents() -> None:
    """Require expected runtime files and reject Bronze entrypoints."""
    missing = [
        path
    ...
    if missing:
        ...
        raise RuntimeError(
            "GHSA Silver Lambda package is missing required files: "
            f"{formatted}"
        )
    ...
    if forbidden:
        ...
        raise RuntimeError(
            "GHSA Silver Lambda package contains forbidden files: "
            f"{formatted}"
        )
...
def write_deterministic_zip() -> None:
    ...
    with ZipFile(
        ...
        for path in files:
            ...
            info = ZipInfo(filename=relative_path, date_time=ZIP_TIMESTAMP)
...
def validate_lambda_size_limit(uncompressed_size: int) -> None:
    ...
    if uncompressed_size > LAMBDA_UNZIPPED_LIMIT_BYTES:
        raise RuntimeError(
            "GHSA Silver Lambda package exceeds the 250 MiB uncompressed limit: "
            f"{uncompressed_size} bytes."
        )


def main() -> None:
    """Build and report the deterministic GHSA Silver Lambda package."""
    prepare_directories()
