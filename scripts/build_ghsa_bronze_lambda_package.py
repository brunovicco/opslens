"""Build the deterministic AWS Lambda package for GHSA Bronze ingestion."""
...
BUILD_DIR = PROJECT_ROOT / "build" / "lambda" / "ghsa-bronze"
PACKAGE_DIR = BUILD_DIR / "package"
REQUIREMENTS_FILE = BUILD_DIR / "requirements.txt"

DIST_DIR = PROJECT_ROOT / "dist"
ARTIFACT_PATH = DIST_DIR / "opslens-ghsa-bronze.zip"
...
REQUIRED_PACKAGE_FILES = (
    ...
    Path("opslens/ingestion/ghsa/runtime_config.py"),
    Path("opslens/ingestion/ghsa/runtime_composition.py"),
    Path("opslens/ingestion/ghsa/adapters/inbound/invocation.py"),
    Path("opslens/ingestion/ghsa/adapters/outbound/github_api.py"),
    Path("opslens/ingestion/ghsa/adapters/outbound/s3_bronze.py"),
    Path("opslens/ingestion/ghsa/adapters/outbound/secrets_manager.py"),
    Path("opslens/ingestion/ghsa/application/runtime.py"),
...
def copy_application_source() -> None:
    """Copy GHSA ingestion and observability runtime source."""
...
def validate_package_contents() -> None:
    """Require all files needed by the manual GHSA Bronze Lambda path."""
    missing = [
        path
    ...
    if missing:
        ...
        raise RuntimeError(
            "GHSA Bronze Lambda package is missing required files: "
            f"{formatted}"
        )
...
def artifact_sha256() -> str:
    """Return the SHA-256 of the exact deployment artifact bytes."""
...
def package_file_count() -> int:
    """Return the number of staged runtime files."""
    return sum(
        1
        for path in PACKAGE_DIR.rglob("*")
        if path.is_file()
...
def main() -> None:
    """Build and report the deterministic GHSA Bronze Lambda artifact."""
    prepare_directories()
    ...
    if compressed_size > LAMBDA_DIRECT_UPLOAD_LIMIT_BYTES:
        raise RuntimeError(
            "GHSA Bronze Lambda package exceeds the 50 MiB compressed reference limit: "
    ...
    print(f"content_addressed_key=lambda/ghsa-bronze/{sha256}.zip")
