...
def _verify_artifact(root: Path, artifact: dict[str, object]) -> tuple[str, int]:
    ...
    expected_key = f"lambda/public-analysis/{role}/sha256={digest}/opslens-public-async-{role}.zip"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="opslens-gate19-5-a-") as first_dir, tempfile.TemporaryDirectory(
        prefix="opslens-gate19-5-b-"
    ) as second_dir:
        ...
        for role in sorted(_EXPECTED_ROLES):
            ...
            if (first_digest, first_bytes) != (second_digest, second_bytes):
                raise Gate19_5ArtifactBuildError(f"{role} artifact metadata differs across rebuilds")
