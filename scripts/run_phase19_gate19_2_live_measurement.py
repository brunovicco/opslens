"""Run one human-operated Gate 19.2 representative workload and persist bounded evidence."""
...
def _require_frozen_operator_inputs(args: argparse.Namespace) -> None:
    ...
    if args.region != BEDROCK_SYNTHESIS_REGION:
    ...
    if args.run_id != args.run_id.strip() or len(args.run_id) > 256:
        raise LiveMeasurementCliError("run-id must be normalized and at most 256 characters")
    if _GIT_SHA_RE.fullmatch(args.opslens_commit_sha) is None:
...
def _github_token(environment_name: str | None) -> str | None:
    """Read an optional token through an explicit environment indirection only."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Cross the human boundary once; no CI workflow invokes this entrypoint."""
