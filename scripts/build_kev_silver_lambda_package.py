"""Build the deterministic AWS Lambda deployment package for KEV Silver."""
...
BUILD_DIR = PROJECT_ROOT / "build" / "lambda" / "kev-silver"
PACKAGE_DIR = BUILD_DIR / "package"
REQUIREMENTS_FILE = BUILD_DIR / "requirements.txt"

DIST_DIR = PROJECT_ROOT / "dist"
ARTIFACT_PATH = DIST_DIR / "opslens-kev-silver.zip"

PYTHON_VERSION = "3.13"
PYTHON_PLATFORM = "x86_64-manylinux_2_28"
RUNTIME_DEPENDENCY_GROUP = "kev-silver-runtime"
...
SOURCE_MANIFEST = (
    ...
    (
        PROJECT_ROOT / "src" / "opslens" / "ingestion" / "kev" / "__init__.py",
        Path("opslens/ingestion/kev/__init__.py"),
    ),
    (
        PROJECT_ROOT / "src" / "opslens" / "ingestion" / "kev" / "domain",
        Path("opslens/ingestion/kev/domain"),
    ),
    (
    ...
    (
        PROJECT_ROOT / "src" / "opslens" / "transformation" / "kev",
        Path("opslens/transformation/kev"),
    ),
    (
...
REQUIRED_PACKAGE_FILES = (
    Path("opslens/transformation/kev/lambda_handler.py"),
    Path("opslens/transformation/kev/composition.py"),
    Path("opslens/transformation/kev/application/service.py"),
    Path("opslens/transformation/kev/adapters/outbound/parquet.py"),
    Path("opslens/ingestion/kev/domain/parser.py"),
    Path("opslens/shared/observability/powertools.py"),
    Path("pyarrow/__init__.py"),
)

FORBIDDEN_PACKAGE_FILES = (
    Path("opslens/ingestion/kev/lambda_handler.py"),
    Path("opslens/ingestion/kev/composition.py"),
)
...
def main() -> None:
    """Build and report the KEV Silver Lambda deployment artifact."""
    prepare_directories()
