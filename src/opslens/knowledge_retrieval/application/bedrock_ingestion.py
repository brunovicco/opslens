...
def _require_statistics(value: object) -> dict[str, int]:
    ...
    unknown = {
        key
        for key in raw
        if not isinstance(key, str) or key not in _ALLOWED_STATISTICS
    }
    if unknown:
...
def _require_failure_reasons(value: object) -> tuple[str, ...]:
    ...
    if len(items) > 20:
        raise BedrockIngestionValidationError(
            "failure_reasons exceeds the bounded evidence limit"
        )
...
class IngestionJobEvidence:
    ...
    def __post_init__(self) -> None:
        """Validate target identity, status, and bounded statistics evidence."""
        object.__setattr__(
            self,
            "knowledge_base_id",
            _require_trimmed(self.knowledge_base_id, field="knowledge_base_id"),
        )
        object.__setattr__(
            self,
            "data_source_id",
            _require_trimmed(self.data_source_id, field="data_source_id"),
        )
        object.__setattr__(
            self,
            "ingestion_job_id",
            _require_trimmed(self.ingestion_job_id, field="ingestion_job_id"),
        )
        status = _require_trimmed(self.status, field="status")
        if status not in _TERMINAL_SUCCESS | _TERMINAL_FAILURE | _ALLOWED_ACTIVE:
            raise BedrockIngestionValidationError(
                f"unsupported ingestion status {status!r}"
            )
        ...
        object.__setattr__(
            self,
            "failure_reasons",
            _require_failure_reasons(self.failure_reasons),
        )
...
def run_bounded_ingestion(
    ...
    *,
    ...
    if type(max_polls) is not int or not 1 <= max_polls <= 120:
        raise BedrockIngestionValidationError(
            "max_polls must be an integer between 1 and 120"
        )
    ...
    if evidence.knowledge_base_id != kb_id or evidence.data_source_id != ds_id:
        raise BedrockIngestionValidationError(
            "started ingestion evidence does not match requested target"
        )
    ...
    for poll_index in range(max_polls + 1):
        ...
        if evidence.status in _TERMINAL_FAILURE:
            ...
            raise BedrockIngestionFailed(
                f"ingestion job {evidence.ingestion_job_id} ended as "
                f"{evidence.status}: {reasons}"
            )
        ...
        if evidence.knowledge_base_id != kb_id or evidence.data_source_id != ds_id:
            raise BedrockIngestionValidationError(
                "polled ingestion evidence does not match requested target"
            )

    raise BedrockIngestionTimeout(
        f"ingestion job {evidence.ingestion_job_id} did not reach a terminal state "
